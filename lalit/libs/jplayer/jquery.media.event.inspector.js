...
(function($, undefined) {
	...
	$.mediaEventInspector.checkBuffering = false;
			...
			*/
			...
			$.each($.mediaEventInspector.event, function(eventName,eventType) {
				...
				config.mediaElementJq.bind(eventType + ".mediaEventInspector", function(e) {
					...
					setTimeout(function() {
						config.eventOccurrence[e.type]--;
						if(e.type == "timeupdate" && $.mediaEventInspector.checkBuffering == true){
							//console.log("config.eventOccurrence"+config.eventOccurrence);
							//console.log(e.type + " (" + config.eventOccurrence[e.type] + ")");
							//console.log(config.eventOccurrence[e.type]);
							if(config.eventOccurrence[e.type] <= 0){
						...
						if(e.type == "play"){
							...
							$.mediaEventInspector.checkBuffering = true;
						}
						if(e.type == "pause"){
