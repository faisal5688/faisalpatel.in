...
MultiOptSingleLevelYesNo.checkAnswer	=	function(evt){	
	...
	if(!$(this).hasClass("disabled")){
		...
		var totalTry = SingleLevelYesNoData.data.total_try;

		if(SingleLevelYesNoData.data.response=="specific"){
			
		if(optId == correctOption){
				feedbackStr    = SingleLevelYesNoData.data.quetionOptFeedback[MultiOptSingleLevelYesNo.currentId].option[(optId)].content;
				audioFileName  = SingleLevelYesNoData.data.quetionOptFeedback[MultiOptSingleLevelYesNo.currentId].option[(optId)].audio;
	//option[(optId-1)] change as per the feedback option//			
				...
				feedbackStr    = SingleLevelYesNoData.data.quetionOptFeedback[MultiOptSingleLevelYesNo.currentId].option[(optId-1)].content;
				audioFileName  = SingleLevelYesNoData.data.quetionOptFeedback[MultiOptSingleLevelYesNo.currentId].option[(optId-1)].audio;
				$(this).addClass("incorrect disabled incorrectTry");
				//$(this).addClass("incorrectTry");
			}
		}else{
		...
		var totalCount = $('.incorrectTry').length;
		if(totalCount == totalTry)
		{
			$(".clk_option").addClass("disabled");
			feedbackStr    = SingleLevelYesNoData.data.quetionOptFeedback[MultiOptSingleLevelYesNo.currentId].option[(correctOption-1)].content;
			eventMgr.dispatchCustomEvent(MultiOptSingleLevelYesNo,"templateActivityCompleted","","");
		}	
