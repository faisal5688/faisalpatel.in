...
TabObj_v1.initTabs = function(data) {
	...
	for (var t = 0; t < TabData.tabs.length; t++) {
		...
		panelMc.find('#tabTitle').html(TabData.tabs[t].label);
		panelMc.find('#tabContent').append(TabData.tabs[t].content);

		//$("#tabContainer #tabHolder").append(panelMc);
		$("#panelContainer").append(panelMc);
	...
	$(".contentWrap").prepend("<div class='page_title'></div>");
...
TabObj_v1.tabClicked = function(evt, ui) {
	...
	//tab_tick
	//$(evt.target).parent().addClass('ui-state-visited');
	//evt.preventDefaults();
}
...
TabObj_v1.addListeners = function() {
	if (!TabObj_v1.activityInitialized) {
		...
		for (var t = 1; t <= TabData.tabs.length; t++) {
			var tab = $("#tabHolder ul").find("#tab-" + t);
			tab.on("click", TabObj_v1.tabClicked);

			$('#tab_' + t).find('#tabPopupCloseBtn').on('click', function() {
				$(this).parent().hide();
				
				//setTimeout(function(){
				$( ".tabsHolder" ).tabs({collapsible: true, active: false });
