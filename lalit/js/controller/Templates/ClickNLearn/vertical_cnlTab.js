...
vertical_cnlTab.initTabs=function(data){	
	...
	$("#cnl_v2_Holder li").removeClass("ui-tabs-active").removeClass("ui-state-active");
	...
	for(var t=0; t<vertical_cnlTab_Data.data.cnl_v2_Items.length; t++){
		...
		//var _str = (t == 0) ? 'class="cnl_v2_tickShow"' : '';
		_str = "";
		$($( "#cnl_v2_Holder ul li")[t]).find("a").append('<div id="cnl_v2_tick' + (t + 1) + '" ' + _str + '></div>');
		$($( "#cnl_v2_Holder ul li")[t]).attr("id","tab-"+(t+1));
	}
	$( "#cnl_v2_Container").html("");
...
vertical_cnlTab.cnl_v2_Clicked=function(evt,ui){
	$('#cnl_v2_Holder').find('.BlankImg').hide();
	...
	if(vertical_cnlTab.currentTabId != tabId){
		$($( "#cnl_v2_Holder ul li")[tabId-1]).addClass("ui-tabs-active");
		...
		if(vertical_cnlTab_Data.data.cnl_v2_Items[tabId-1].markVisit=="init"){
			...
			if(numLength==vertical_cnlTab_Data.data.cnl_v2_Items.length){
			...
			eventMgr.dispatchCustomEvent(vertical_cnlTab,"tabSelected","",{"tabId": tabId});
			AudioController.playInternalAudio(vertical_cnlTab_Data.data.cnl_v2_Items[tabId-1].audio);
		}else{
