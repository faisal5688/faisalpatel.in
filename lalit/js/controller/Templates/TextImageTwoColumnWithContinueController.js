...
TextImageObj.showSection = function(){
	...
	for(var t=0; t<TextImageData.sectionArr.length; t++){
		var colContainer = maincontainer.find(".tempColumn1").clone();
		colContainer.removeClass("tempColumn1");	
		colContainer.attr("id", "ContHolder"+t);
		colContainer.find("#TextHolder1").html(TextImageData.sectionArr[t].content);
		$(".firstdataContainer").append(colContainer);
	}
	for(var t=0; t<TextImageData.sectionArr1.length; t++){
		...
		colContainer.find("#HeadingContainer").html(TextImageData.sectionArr1[t].heading);
		colContainer.find("#TextHolder").html(TextImageData.sectionArr1[t].content);
		$(".secondDataContainer").append(colContainer);
	}
	$(".developmentcost_first").append(TextImageData.continueContent[0].content);
	$(".developmentcost_data_details").append(TextImageData.continueContent[1].content);
}
