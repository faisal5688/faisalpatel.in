	...
	for(var d=0; d< dataRef.length; d++){
		switch(dataRef[d].type){
			case "input":
				...
				if(ActivityObj.mainContainer.find("#input_"+(d+1)).val().toLowerCase() == dataRef[d].answerSecond.toLowerCase()){
					correctCounter++;
				}
			break;
			case "dropDown":				
				...
				if(ActivityObj.mainContainer.find("#input_"+(d+1)).prop("selectedIndex") == dataRef[d].answerSecond){
					correctCounter++;
				}				
			break;
		}
