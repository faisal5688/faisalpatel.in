...
ActivityObj.initActivity=function(data){
	...
	//initialize Feedback	
	Feedback.init($("#mcmsFeedbackBox"), $("#activityContainer"));
	$("#mcmsFeedbackBox").find("#feedbackClose").on("click",ActivityObj.closeFeedback);	
...
ActivityObj.initInternalActivity=function(data){
	...
	for(var d=0; d<data.length; d++){
		...
		str += '<td><div class="quest_text">'+data[d].questtext+'</div></td><td><div id="tick_'+(d+1)+'" class="tick"></div><div class="selectCont"><select id="input_'+(d+1)+'">';
		switch(data[d].type){			
			case "dropDown":
...
ActivityObj.showFeedback=function(result){
	...
	if(result){		
		Feedback.showFeedback(ActivityData.feedback.correct);
		ActivityObj.markComplete();		
	}
	else if(ActivityObj.currentAttempt < ActivityData.maxAttempts){
		Feedback.showFeedback(ActivityData.feedback.incorrect);
		$(".soln_inst").hide();
		$(".close_inst").fadeIn();		
	}else{
...
ActivityObj.showSoluton = function(){
	...
	$("#activityHolder .tick").removeClass('inputCross').show();
	$(".submitBtn input[type='button']").css("cursor","default");
}
