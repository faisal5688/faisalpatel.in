...
MCMS.initMCMS = function(data) {
	...
	$('#feedbackOptionSpecificClose').on('click',MCMS.closeOptionSpecificFeedback);
...
MCMS.showFeedback = function(result) {	
	console.log(MCMS.correctAnswer);
	var ans = "<ul>";
	for ( c = 0; c < MCMS.questionData.options.length; c++) {		
...
MCMS.onGlass_clicked = function(evt){	
	var clicked = $(evt.target).parent();
	var option_num = parseInt(clicked.attr("id").split('_')[1],10)-1;
	
	$('#feedbackOptionSpecificContent').html(MCMS.questionData.specificResponse.response[option_num].content);
	$('#mcmsOptionSpecificFeedbackBox').show();
}

MCMS.closeOptionSpecificFeedback = function(evt){
	$('#mcmsOptionSpecificFeedbackBox').hide();
}
