...
MCSS.letters = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"];
...
MCSS.showFeedback = function(result, selectedOption) {
    ...
    if (MCSS.questionData.responseType == 'specific') {
        var _fTxt = MCSS.questionData.specificResponse.response[selectedOption - 1].content;
            if (result != 'Correct') {
                $('.hideinIncorrect').hide()
                var tempArr = [];
                tempArr = MCSS.correctAnswer.concat()
                _fTxt += '<br/>' + DataManager.templateXMLData.data.MCQ.feedback.specificResponse.response[tempArr[0] - 1].showAnswer;
            }else{
                $('.hideinIncorrect').hide()
            }
...
MCSS.sendCMIInteractionData = function(result, selectedOptName, correctOptName) {
    ...
    if (_tocData.isInteraction == 'true') {
        ...
        var _courseId = DataManager['courseDetail']['id'] || "FIRM-19-WB-223-R"
        ...
        try {
            ...
            SCORMAPIService.setInteractionData(questionID, cmiType, timeStr, latency, weightage, correctResponse, userResponse, userResult, KCNumber, DataManager.objectivePageArr.length);
