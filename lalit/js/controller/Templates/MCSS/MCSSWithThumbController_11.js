...
MCSS.submitClicked = function(evt) {
    MCSS.disableOptions();
    MCSS.chkUserAnswer();
    MCSS.mainContainer.find("#mcssSubmitBtn").hide();
    // MCSS.updateAudio(0);
};
...
MCSS.showFeedback = function(result, selectedOption) {
    ...
    var _mcssOption = $(".iOption.mcssOption");
    _mcssOption.addClass('answered');

    var _optionTexts = _mcssOption.find(".optionText")
    var _len = MCSS.questionData.options.length;
    var _response = MCSS.questionData.specificResponse.response
    for (i = 0; i < _len; i++) {
        var _fTxt = _response[i].content;
        $(_optionTexts[i]).find('.txt').html(_fTxt)
    }
...
MCSS.sendCMIInteractionData = function(result, selectedOptName, correctOptName) {
    ...
    if (_tocData.isInteraction == 'true') {
        ...
        var _courseId = DataManager['courseDetail']['id'] || "FIRM-20-WB-225"
